...
```{r all_data, message=FALSE, warning=FALSE}
aux.data <- loan.data %>% 
  ungroup() %>%
...
```{r rw_level_histogram, warning=FALSE, message=FALSE}
aux.data %>%
  ggplot(data = filter(aux.data, RW<1), mapping = aes(x = RW)) +
  geom_histogram() +
  theme_bw()+
#scale_y_continuous(expand = c(0,0)) +
 #  scale_x_continuous(expand = c(0,0)) +
...
aux3 = ""
aux.data %>%
  ggplot(mapping = aes(y = RW, x = year)) +
  geom_line(data = filter(aux.data, country_code_lender == aux1 ), 
            color = "black", linetype = 5, size = 1, aes(group = bank_name))+
  geom_line(data = filter(aux.data, country_code_lender == aux2 ), 
            color = "gray50", linetype = 1, size = 1, aes(group = bank_name))+
#  geom_line(data = filter(aux.data, country_code_lender == aux3 ), 
#            color = "black", linetype = 2, size = 1, aes(group = bank_name))+
  theme_bw()+
#scale_y_continuous(expand = c(0,0)) +
   scale_x_continuous(expand = c(0,0)) +
...
```{r intro_by_year, warning=FALSE, message=FALSE}
aux.data %>% 
  select(bvdid, bank_name, year, IRB) %>%
  arrange(bvdid, year) %>%
...
aux.data %>%
  ggplot(mapping = aes(y = RW, x = year)) +
  geom_line(data = filter(aux.data, country_code_lender == aux1 ), 
            color = "black", linetype = 1, size = 1, aes(group = bank_name))+
  geom_line(data = filter(aux.data, country_code_lender == aux2 ), 
            color = "gray50", linetype = 1, size = 1, aes(group = bank_name))+
#  geom_line(data = filter(aux.data, country_code_lender == aux3 ), 
#            color = "black", linetype = 2, size = 1, aes(group = bank_name))+
...
```{r lender_country_total_amount, warning=FALSE, message=FALSE}
aux.data <- loan.data %>% 
  filter(!is.na(bvdid)) %>%
...
country.aux2  = "GB"
aux.data %>%
 ggplot(data = filter(aux.data, country_code_lender != country.aux1, country_code_lender != country.aux2), 
        mapping = aes(x = year, y = lender_amount)) +
  geom_line(color = "gray70", aes(group = country_code_lender)) +
  geom_line(data = filter(aux.data, country_code_lender == country.aux1 ),
            color = "black", linetype = 1, size = 1)+
  geom_line(data = filter(aux.data, country_code_lender == country.aux2 ),
...
```{r lender_country_cross_amount, warning=FALSE, message=FALSE}
aux.data <- loan.data %>% 
  filter(!is.na(bvdid), country_name_borrower != country_name) %>%
  group_by(country_code_lender, year)%>%
  summarise(lender_amount = sum(lender_amount, na.rm = TRUE)/1000)
...
```
```{r borrower_country_total_amount, warning=FALSE, message=FALSE}
aux.data <- loan.data %>% 
  filter(!is.na(bvdid)) %>%
  group_by(country_name_borrower, year)%>%
  summarise(lender_amount = sum(lender_amount, na.rm = TRUE)/1000)

country.aux1  = "United States"
country.aux2  = "United Kingdom"
aux.data %>%
  ggplot(data = filter(aux.data, country_name_borrower != country.aux1, country_name_borrower != country.aux2), 
         mapping = aes(y = lender_amount, x = year)) +
  geom_line(color = "gray70", aes(group = country_name_borrower)) +
  geom_line(data = filter(aux.data, country_name_borrower == country.aux1 ), 
            color = "black", linetype = 1, size = 1.1)+
  geom_line(data = filter(aux.data, country_name_borrower == country.aux2 ), 
...
```{r borrower_country_cross_amount, warning=FALSE, message=FALSE}
aux.data <- loan.data %>% 
  filter(!is.na(bvdid), country_name_borrower != country_name) %>%
  group_by(country_name_borrower, year)%>%
  summarise(lender_amount = sum(lender_amount, na.rm = TRUE)/1000)
...
loan.data %>%
        filter(year>1994, country_name_borrower != country_name) %>% 
  select(
  log.lender_amount,
  RW,
  basel,
...
reg.list[[1]] <- felm(log.lender_amount ~ RW  | as.factor(bvdid) + tranche_id | 0 | 0,
                      data = filter(loan.data, country_name_borrower != country_name, year > 1994))
...
se.list <- list()
reg.list[[1]] <- felm(log.lender_amount ~ RW  | as.factor(bvdid) + tranche_id | 0 | 0,
                      data = filter(loan.data, country_name_borrower != country_name, year > 2007))
se.list[[1]]  <- summary(reg.list[[1]], robust = TRUE)$coefficients[,2]
...
```{r regressions_irb, echo=FALSE, }

# Run the models
reg.list <- list()
se.list <- list()
reg.list[[1]] <- felm(log.lender_amount ~ RW  | as.factor(bvdid) + tranche_id | 0 | 0,
                      data = filter(loan.data, country_name_borrower != country_name, IRB == 1))
se.list[[1]]  <- summary(reg.list[[1]], robust = TRUE)$coefficients[,2]


reg.list[[3]] <- felm(log.lender_amount ~ 1 | as.factor(bvdid) + tranche_id| (RW ~ mean.RW_adj) | 0,
                      data = filter(loan.data, country_name_borrower != country_name, IRB == 1))
se.list[[3]]  <- summary(reg.list[[3]], robust = TRUE)$coefficients[,2]
...
```{r tables_aux_irb, echo=FALSE}

# Dependent variables
dep.text <- c("$\\text{Log loan}_{l,i}$", "$\\text{RW}_{i,t}$", "$\\text{Log loan}_{l,i}$","$\\text{RW}_{i,t}$", "$\\text{Log loan}_{l,i}$")

# Independent variables
ind.text <- c( "$\\text{RW}_{i,t}$", "$\\overline{\\text{RW}}_{c,t}$",  "$\\widehat{\\text{RW}}_{c,t}$")
...
```{r table_regression_irb, eval=TRUE, echo=FALSE, results='asis', warning=FALSE, message=FALSE}
stargazer(reg.list,
          title = "Results",
          table.layout = "n=c=d#-t-as-",
          notes = "\\parbox[t]{14cm}{The table shows the effect of risk-weights on corporate lending. The dependent variable is the logarithm of the amount lent by the bank in a facility. The independent variable is the average risk-weight of the bank's corporate portfolio in the year that the facility was issued. All regressions include facility and bank fixed effects. Robust standard errors are reported in parentheses. *, **, and *** indicate statistical significance at the 10\\%, 5\\%, and 1\\% level respectively.}",
